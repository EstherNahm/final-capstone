	...
	<div class="row">
    	...
    	<div class="col-sm-8 col-md-8">
     	  <div class="favoritescard">
				...
				<h4 id="favoritesheader">Favorites</h4>
				</div>
				...
				<c:forEach var="restaurant" items="${restaurants}">
					<div class="card-header">
							<div class="card-header-favorites">
								<h5 id="favoritesheader1"><i class="fa fa-cutlery favoritesicon" aria-hidden="true"><span class="welcomefont">&nbsp;${restaurant.name}</span></i></h5>
								<c:url var="deleteForm" value="/deleteFavorite" />
								<form action="${deleteForm}" method=POST>
								<input type="hidden" name="restaurantId" value="${restaurant.id}"/>
								<input type="hidden" name="CSRF_TOKEN" value="${CSRF_TOKEN}"/>
								<button type="submit" class="btn commentbtn"
													style="background-color: #b2cfb2;; width: 40px; " id="trash"><i class="fa fa-trash fa-lg" aria-hidden="true"></i></button>
								</form>
				...
				<div class="favorites-picture">
					...
					<div class="favorites-icon" style="margin-left: 5%;">
		 			...
		 			<i class="fa fa-home favoritesicon fa-sm"></i><a>&nbsp;${restaurant.address }</a><br><br>
		 			<i class="fa fa-phone favoritesicon fa-sm"></i><a>&nbsp;${restaurant.displayPhone}</a>
		 			</p>
					
					
					
			<!-- Button to Open the Modal -->
<button type="button" class="btn btn-primary" id="favoritebutton" data-toggle="modal" data-target="#myModal">
  ...
  <div class="modal-dialog">
    <div class="modal-content" style="background-color: rgb(169,169,169); padding: 2%;">
      ...
      <div class="modal-header">
		...
		<input type="hidden" name="CSRF_TOKEN" value="${CSRF_TOKEN}"/>
		<input type="hidden" name="restaurantId" value="${restaurant.id}"/>
		<input type="hidden" name="destination" value="redirect:/viewFavorites"/>
      <!-- Modal body -->
      ...
      <div class="row" style="margin-left: 3%; margin-right: 3%;">
				...
				<p class="welcomefont"><i class="fa fa-user favoritesicon fa-sm" aria-hidden="true"></i>&nbsp;${visitReview.username}<br>
				${visitReview.dateOfVisit}</p>
				<c:if test="${visitReview.isUsersReview}">
								<form action="${deleteAction }"><span>
								<input type="hidden" name="CSRF_TOKEN" value="${CSRF_TOKEN}" />
								<input type="hidden" name="visitId" value="${visitReview.visitId}" />
								<input type="hidden" name="restaurantId" value="${restaurant.id}"/>
								<input type="hidden" name="destination" value="redirect:/viewFavorites"/>
								<button type="submit" class="btn commentbtn"
													style="background-color: #b2cfb2;; width: 40px; " id="trash"><i class="fa fa-trash fa-lg" aria-hidden="true"></i></button></span>
								</form>
								</c:if>
				</div>
				<div class="col-sm-1 col-md-1"></div>
				...
				<div class="col-sm-8 col-md-8">
								...
								</c:forEach><c:url var="deleteAction" value="/deleteReview"/>
								
								
								<br>
								
											<p class="welcomefont">${visitReview.review}</p>
